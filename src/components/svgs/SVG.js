import react from "react";

import Mainsvg from "./mainsvg";
import Proj1 from "./proj1big";
import Projsmall from "./proj1small";
import Proj2 from "./proj2big";
import Proj2small from "./proj2small";
const SVG = {
  Mainsvg: <Mainsvg height="100%" width="100%" />,
  Proj1: <Proj1 height="100%" width="100%" />,
  Projsmall: <Projsmall height="100%" width="100%" />,
  Proj2: <Proj2 height="100%" width="100%" />,
  Proj2small: <Proj2small height="100%" width="100%" />,
};
export default SVG;
